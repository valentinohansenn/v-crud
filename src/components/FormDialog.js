...
export default function FormDialog(props) {
    ...
    const submitHandler = async () => {
        setOpen(false);
        const json = props.data !== undefined ? await props.action(form, id) : await props.action(form);
        toast(`You have successfully edited the product named ${json.title} that was listed with a price tag of $${json.price}.`, {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button
                    variant="outline"
                    className="text-white rounded-full font-bold font-sans px-0 py-0 border-2 border-black bg-black h-full w-64 flex justify-center hover:bg-gray-700 hover:text-white focus:outline-none"
                >
                    {props.title}
                </Button>
            </DialogTrigger>
